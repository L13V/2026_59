	...
	public class ShotCalculator {
		...
		*/
		...
		public void update(double loopTime) {
			...
			double chassisRadAccel = (chassisSpeeds.omegaRadiansPerSecond - chassisAngleRateLast) * (1000 / loopTime);
			chassisAngleRateLast = chassisSpeeds.omegaRadiansPerSecond; // store newest value for the rate thing.
			...
			angleRate = DataProcessing.rawToSmooth(5, angleRate,
					(gyroAngleRateRaw * angleBias) + (gyroAngleRateRaw * (1 - angleBias)));

			angleAccel = DataProcessing.rawToSmooth(5, angleAccel,
					(gyroRadAccel * angleBias) + (gyroRadAccel * (1 - angleBias)));

			double shooterFieldAngle = Math.toRadians(250) + robotState.getRotation().getRadians()
			...
			* Fish
			*/
			double sanetizedHoodAngle = DataProcessing.sanitize(last.hoodAngle + hoodMinAngle, hoodMinAngle, hoodMaxAngle,
					hoodAngleFeedback);
			// sanetizedHoodAngle = Units.degreesToRadians(last.hoodAngle);
			...
			// Logger.recordOutput("test", robotPoseDynamicXY);

			double turretAngle = getAngleToTarget(new Pose3d(robotPoseDynamicXY), targetPose).getDegrees()
		...
		public void publishShotParameters() {
			...
			// Logger.recordOutput("RobotState/ZoneName", zone != null ? zone.name() :
			// "hello!");
