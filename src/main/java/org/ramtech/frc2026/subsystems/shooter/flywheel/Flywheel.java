...
public class Flywheel extends ShooterSubsystem {
	...
	private final FlywheelIOInputsAutoLogged inputs = new FlywheelIOInputsAutoLogged(); // the truth: what the motor
																						// sees
	...
	private final Alert rightSideDisconnected = new Alert("Right Flywheel Motor Disconnected",
	...
	public void periodic() {
		...
		rightSideDisconnected
				.set(Robot.showHardwareAlerts() && !rightSideDebouncer.calculate(inputs.rightSideConnected));
	}
		...
		synchronized (outputsLock) {
			if (outputs.setpointSource == FlywheelIOSetpointSource.SHOT_CALCULATOR) {
				if (shotCalculation.isValid()) {
					...
					outputs.feedForward = shotCalculation.flyWheelFeedForward();
				}
			}
	...
	public double getAverageVelocity() {
		return (Math.abs(inputs.leftSideVelocity) + Math.abs(inputs.rightSideVelocity)) / 2.0;
	}
}
